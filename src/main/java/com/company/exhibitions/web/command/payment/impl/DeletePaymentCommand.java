...
public class DeletePaymentCommand extends PaymentCommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!Objects.isNull(attribute)) {
            ...
            controllerExecutor.perform(() -> {
                Payment payment = paymentService.findPaymentById(extractParameters(request));
            ...
            if(!Objects.isNull(request.getAttribute(CustomRequestAttributes.PAYMENT.name().toLowerCase()))){
                controllerExecutor.perform(() -> paymentService.deletePayment(parameters));
            }
            return PageManager.getProperty("page.admin");
        }
    }
