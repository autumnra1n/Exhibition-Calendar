...
@AdminCredentials
public class InsertRoleCommand extends Command {
    ...
    public InsertRoleCommand(){
        this.controllerValidator = new RoleNameValidator(null);
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (Objects.nonNull(attribute)) {
            ...
            if (Objects.isNull(request.getAttribute(CustomRequestAttributes.ROLE.name().toLowerCase()))) {
                super.getControllerExecutor().perform(() -> roleService.insertRole(parameters));
            }
            return pageAdmin;
        }
    }
